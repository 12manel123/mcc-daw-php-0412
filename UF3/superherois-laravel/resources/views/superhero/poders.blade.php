...
<form action="{{route('superhero.canviarpoders',[$superhero->id,$powers->id])}}" method="POST">
	@csrf

	<td><input type="number" name="nivell" value="{{$powers->level->amount}}"></td>

	<td><input type="submit" value="Canviar" > </form>| <a href="{{route('superhero.eliminarpoders',[$superhero->id,$powers->id])}}">Eliminar</a></td>
...
<div id="poder">
<h3>Llista poders</h3>
<form method="POST" action="{{route('superhero.afegirpoder',$superhero->id)}}">
	@csrf
